...
@Component({
  selector: 'app-nfc-guard-patrol-supervisor-manage-routes',
  templateUrl: './nfc-guard-patrol-supervisor-manage-routes.component.html',
  styleUrls: ['./nfc-guard-patrol-supervisor-manage-routes.component.scss']
})
export class NfcGuardPatrolSupervisorManageRoutesComponent implements OnInit {
  ...
  selectSecurityGroup(group) {
    ...
    console.log('selectSecurityGroup route', group);
    ...
    // if (this.selectedPatrollingGroup.PatrollingEmployees.length > 0) {
    //   this.getAllGuards();
    //   this.selectedTab = 'groupEmployees';
    // }
    ...
    this.nfcGuardPatrolBase.postPatrollingTaskChangeIsType(
      {
        ...
        'SecurityTaskType': updateTask.SecurityTaskTypeId
      }
    ).subscribe(value => {
