...
@Component({
  selector: 'app-dutyroster-supervisor-create-shifts',
  templateUrl: './dutyroster-supervisor-create-shifts.component.html',
  styleUrls: ['./dutyroster-supervisor-create-shifts.component.scss']
})
export class DutyrosterSupervisorCreateShiftsComponent implements OnInit {
  // =============================================================
  // UI Realted
  ...
  showRollcalLoader: boolean;
  enableRollcalButton: boolean;
  // =============================================================
  topButtons: any;
  // Shift Groups ================================================
  shiftGroups: any = [];
  ...
  // =============================================================
  dutyRosterData: any = { lastYear: 0, thisYear: 0 };
  // =============================================================
  week: any;
  weekDetails: any;
  ...
  selectedStaff: any;
  //
  showCancelledHistory: boolean = false;
  //
  showDeletedEmployees: boolean = false;

  selectedWeekNo: any;

  selectedRollCallDetails: any;
  ...
  resetRollCallDetails() {
    this.selectedRollCallDetails = null;
  }
  ...
  toggleShowDeletedEmployees() {
    console.log('toggleShowDeletedEmployees -> this.showDeletedEmployees :: ',this.showDeletedEmployees)
    if (this.showDeletedEmployees) {
      this.showDeletedEmployees = false;
    } else {
      this.showDeletedEmployees = true;
    }
    console.log('toggleShowDeletedEmployees -> this.showDeletedEmployees :: ',this.showDeletedEmployees)
  }
  ...
  getAllShiftGroups(pageSize, pageNum) {
    ...
    this.assignedShiftGroups.PageSize = pageSize;
    this.assignedShiftGroups.PageNumber = pageNum;
    this.shiftGroups = [];
    this.dutyRosterBase.postShiftGroupsAssignedToSupervisor(
      {
        'PageSize': this.assignedShiftGroups.PageSize,
        'PageNumber': this.assignedShiftGroups.PageNumber
      }
    ).subscribe(data => {
  ...
  getTopButtonDetails() {
    return new Promise(resolve => {
      this.dutyRosterBase.getCurrentWeekDetails().subscribe(data => {
        this.dutyRosterData.lastYear = data;
        this.dutyRosterData.thisYear = (data.Year - 1);
        if (data.Year === 2021 || (data.Year - 1) === 2021) {
          if (data.WeekNo === 1) {
            this.setWeekDetailsInTopButton(0, (53), (data.Year - 1));
          ...
          if (data.WeekNo === 1) {
            this.setWeekDetailsInTopButton(0, (52), (data.Year - 1));
            this.setWeekDetailsInTopButton(1, (data.WeekNo), (data.Year));
            this.setWeekDetailsInTopButton(2, (data.WeekNo + 1), (data.Year));
            this.setWeekDetailsInTopButton(3, (data.WeekNo + 2), (data.Year));
          } else {
  ...
  newShift(selectedStaff) {
    ...
    this.showShiftEmployeeLoader = true;
    this.dutyRosterBase.postShiftNew(dataBody).subscribe(value => {
      this.showShiftEmployeeLoader = false;
  ...
  private setWeekDetailsInTopButton(buttonIndex, weekNo, year) {
    console.log('setWeekDetailsInTopButton top ', buttonIndex, weekNo, year);
    ...
    console.log('setWeekDetailsInTopButton bottom', weekNo, year);
  }
  ...
  showRollCallDetails() {
    this.showRollcalLoader = true;
    this.dutyRosterBase.getRollCallDetails(this.selectedShiftGroupId, this.selectedWeekNo, this.thisYear).subscribe(resp => {
      this.enableRollcalButton = false;
      this.showRollcalLoader = false;
      console.log('getRollCallDetails', resp);
      this.selectedRollCallDetails = resp;
      this.dutyRosterBase.showCreateNewMsgBox('Hooray!', 'You have Loaded Attendence Data Sucessfully ', 1000);
      // this.weekDetails.push(resp)
    }, (error: Response | any) => {
      this.showRollcalLoader = false;
      return Observable.throw(new Error(error.status));
    });
