...
@Component({
  selector: 'app-external-payment-daily',
  templateUrl: './external-payment-daily.component.html',
  styleUrls: ['./external-payment-daily.component.scss']
})
export class ExternalPaymentDailyComponent implements OnInit {
  showLoader: boolean;
  nowDateTime: any;
  ...
  constructor(
    private xPayBase: ExternalPaymentBaseService) {
  ...
  loadTransactions() {
    ...
    if (!this.selectedDate || !this.selectedOrganization) {
      ...
      this.xPayBase.getTransactions(this.selectedDate, this.selectedOrganization).subscribe(response => {
